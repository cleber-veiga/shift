...
function WorkspaceEditor({
  ...
  const erpDisplayName =
    availableErps.find((erp) => erp.id === workspaceErpId)?.name ??
    (workspaceErpId ? "ERP vinculado" : "Nenhum")
  ...
  return (
    <div className="flex flex-col space-y-4">
      ...
      <div className="flex items-center justify-between rounded-lg border border-border bg-card/50 p-2">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-sm font-bold tracking-tight">{workspaceName}</h1>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {saveError ? (
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <div>
              ...
              <input
                ...
                className="h-8 w-full rounded-md border border-input bg-background/50 px-3 text-xs outline-none transition-all focus:ring-1 focus:ring-primary/20"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                readOnly
                disabled
                className="h-8 w-full rounded-md border border-input bg-muted/30 px-3 text-xs text-foreground/80 outline-none"
              />
              {loadingErps ? <p className="mt-1 text-[10px] text-muted-foreground">Carregando ERPs...</p> : null}
              {erpError ? <p className="mt-1 text-[10px] text-destructive">{erpError}</p> : null}
        ...
        <section className="flex flex-col space-y-3">
          <div className="flex items-center justify-between px-1">
            <div className="flex items-center gap-2 text-muted-foreground">
              ...
              <span className="text-[11px] font-medium">{players.length} concorrentes vinculados</span>
            </div>
            <button
              type="button"
              onClick={openNewPlayerModal}
              className="inline-flex h-7 items-center gap-1.5 rounded-md border border-border bg-background px-3 text-[11px] font-bold transition-all hover:bg-accent"
            >
          ...
          {playersLoading ? (
            ...
            <div className="overflow-auto rounded-lg border border-border bg-card shadow-sm">
              <div className="grid grid-cols-[1fr_180px_80px] items-center border-b border-border bg-muted/20 px-4 py-2 text-[9px] font-bold uppercase tracking-wider text-muted-foreground">
              ...
              <div className="divide-y divide-border">
                {players.map((player) => (
                  <div
                    key={player.id}
                    className="grid grid-cols-[1fr_180px_80px] items-center px-4 py-2 transition-colors hover:bg-muted/10"
                  >
                    <p className="truncate text-[11px] font-semibold text-foreground">{player.name}</p>
                    ...
                    <div className="flex items-center justify-end gap-0.5">
                      ...
                      <button
                        ...
                        className="rounded p-1.5 text-destructive/60 transition-colors hover:bg-muted hover:text-destructive"
                      >
            ...
            <div className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-border bg-card/30 py-12">
              ...
              <p className="text-xs font-semibold text-foreground">Nenhum concorrente</p>
              ...
              <button
                type="button"
                onClick={openNewPlayerModal}
                className="inline-flex h-7 items-center gap-1.5 rounded-md bg-primary px-4 text-[11px] font-bold text-primary-foreground transition-all hover:opacity-90"
              >
                <Plus className="size-3" />
                Adicionar Primeiro
              </button>
          ...
          <div className="w-full max-w-lg rounded-xl border border-border bg-card p-5 shadow-2xl">
            <div className="mb-5 flex items-start justify-between">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-xs font-bold uppercase tracking-tight text-foreground">
            ...
            <form onSubmit={handleSavePlayer} className="space-y-4">
              ...
              {playerFormError ? (
                <p className="rounded-md border border-destructive/20 bg-destructive/10 px-2.5 py-1.5 text-[10px] text-destructive">
                  {playerFormError}
                </p>
              ) : null}
              ...
              <div className="flex justify-end gap-2 pt-1">
                <button
                  ...
                  onClick={closePlayerModal}
                  disabled={savingPlayer}
                  className="inline-flex h-8 items-center justify-center rounded-md border border-border bg-card px-4 text-[11px] font-semibold text-foreground transition-colors hover:bg-accent disabled:opacity-50"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  disabled={savingPlayer}
                  className="inline-flex h-8 items-center justify-center gap-1.5 rounded-md bg-primary px-4 text-[11px] font-bold text-primary-foreground transition-opacity hover:opacity-90 disabled:opacity-50"
                >
...
export default function WorkspaceFormPage({ params }: PageProps) {
  ...
  useEffect(() => {
    ...
    listErps()
      ...
      .catch((err) => {
        if (!active) return
        setErpError(err instanceof Error ? err.message : "Falha ao carregar ERPs.")
      })
      .finally(() => {
        if (!active) return
  ...
  return (
    <WorkspaceEditor
      ...
      workspaceErpId={workspace.erp_id ?? ""}
      organizationName={organizationName}
      availableErps={availableErps}
      loadingErps={loadingErps}
      erpError={erpError}
